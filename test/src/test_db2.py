...
@pytest.mark.parametrize(
    "cr_k,cr_v,pod_v,expected",
    [
        (
            "MIRRORLOGPATH",
            "/mnt/backup/MIRRORLOGPATH",
            "/mnt/backup/MIRRORLOGPATH/NODE0000/LOGSTREAM0000/",
            True,
        ),
        (
            "MIRRORLOGPATH",
            "/mnt/backup/MIRRORLOGPATH",
            "/notcorrect/NODE0000/LOGSTREAM0000/",
            False,
        ),
        (
            "NOTSPECIAL",
...
def test_check_db_cfgs_no_spec():
    with pytest.raises(
        Exception, match="spec.environment.databases not found or empty"
    ):
        db2.check_db_cfgs(dict(), None, None, None)
...
def test_check_db_cfgs_no_environment():
    with pytest.raises(
        ...
        db2.check_db_cfgs(dict(spec=dict()), None, None, None)
...
def test_check_db_cfgs(mocker):
    ...
    assert mock_check_db_cfg.call_args_list == [
        mocker.call(
            dict(name="a"), mock_core_v1_api, "mas_instance_id", "mas_app_id", "primary"
        ),
        mocker.call(
            dict(name="b"), mock_core_v1_api, "mas_instance_id", "mas_app_id", "primary"
        ),
    ]
...
def test_check_db_cfg(mocker):
    ...
    mock_db2_pod_exec_db2_get_db_cfg = mocker.patch(
        "mas.devops.db2.db2_pod_exec_db2_get_db_cfg"
    )
  ...
  """
    ...
    db = dict(
        name=db_name,
        dbConfig=dict(
            APPLHEAPSZ="8192 AUTOMATIC", NOTFOUNDINOUTPUT="XXX", CHNGPGS_THRESH="40"
        ),
    )
...
def test_check_dbm_cfg_no_environment():
    db2_instance_cr = dict(spec=dict())
    assert db2.check_dbm_cfg(db2_instance_cr, None, None, None) == []


def test_check_dbm_cfg_no_instance():
    db2_instance_cr = dict(spec=dict(environment=dict()))
    assert db2.check_dbm_cfg(db2_instance_cr, None, None, None) == []
...
def test_check_dbm_cfg_empty_dbmConfig():
    db2_instance_cr = dict(spec=dict(environment=dict(instance=dict(dbmConfig=dict()))))
    assert db2.check_dbm_cfg(db2_instance_cr, None, None, None) == []


def test_check_dbm_cfg(mocker):

    mock_db2_pod_exec_db2_get_dbm_cfg = mocker.patch(
        "mas.devops.db2.db2_pod_exec_db2_get_dbm_cfg"
    )
...
def test_check_reg_cfg_empty_registry():
    db2_instance_cr = dict(spec=dict(environment=dict(instance=dict(registry=dict()))))
    assert db2.check_reg_cfg(db2_instance_cr, None, None, None) == []
...
@pytest.mark.parametrize(
    "test_case_name, expected_failures",
    [
...
def test_validate_db2_config(test_case_name, expected_failures, mocker):
    ...
    with open(
        os.path.join(
            current_dir, "..", "test_cases", test_case_name, "db2uinstance.yaml"
        ),
        "r",
    ) as f:
    ...
    try:
        with open(
            os.path.join(
                current_dir, "..", "test_cases", test_case_name, "db2getdbcfg.txt"
            ),
            "r",
        ) as f:
    ...
    try:
        with open(
            os.path.join(
                current_dir, "..", "test_cases", test_case_name, "db2getdbmcfg.txt"
            ),
            "r",
        ) as f:
